...
- The Task 2 write-amplification incident fix may ship independently after its own QA gate so daily Durable Object consumption stops immediately. One additional server-only exception is required before any Defense client ships: Task 8 must first deploy a `surface=defense` fail-closed compatibility fence with no Defense UI or protocol enabled, then record that version as the rollback baseline. All visible Rally/Defense separation, static-Defense removal, and UI work after that uses one atomic production release; never remove the old Rally Defense UI before the new Defense page and routing are ready in the same deployable commit.
...
- [x] **Step 1: Run GitNexus impact for `ensureAudio`, `audioAlive`, `paintAudioStatus`, `keepAwake`, `scheduleBeeps`, `schedulePrepareCue`, `reconcileCues`, and `rebookCuesOnDrift`.** The report confirmed the expected CRITICAL readiness/audio fan-out and HIGH cue-scheduling blast radius; Rally characterization remained the migration gate.
- [x] **Step 2: Write failing engine tests.** The first run failed because all three shared modules were absent. The final suites cover explicit enablement, carrier recovery, exact readiness truth, exact SFX paths, cue identity/retry/cancellation, node cleanup, clock-edge republishing, suspended-context behavior, and transactional rollback after partial Web Audio construction.
- [x] **Step 3: Implement the audio engine and readiness projection.** The existing 40Hz carrier, MediaSession/Wake Lock behavior, 80ms carrier-loss confirmation, 250ms resume recovery, and `/sfx/{en,zh}_{5,4,3,2,1,go}.mp3` assets remain unchanged.
- [x] **Step 4: Implement the generic cue scheduler.** Cue policy remains in the Rally adapter; the scheduler accepts explicit absolute-time events, reconciles future work, and releases every oscillator/gain node on cancellation, completion, or failure.
- [x] **Step 5: Adapt Rally one function at a time.** Each replacement passed the audio/readiness characterization gate before the duplicate inline implementation was removed. The final focused/integrated audio gate passed 89/89.
- [x] **Step 6: Prove commander and participant routing remain unchanged.** Commander silence, selected-player routing, Rally wiring, and delivery wiring all pass unchanged.
- [x] **Step 7: Run all unit and browser-core regressions.** Final evidence: `npm test` 467/467, Delivery 125/125, Triple 217/217, Chromium/Firefox/WebKit Core + Compatibility passed serially, and the two-page background carrier long-run passed.
- [x] **Step 8: Run GitNexus change detection and commit.** The expected CRITICAL shared audio/socket fan-out was reviewed with zero Critical/Important code-review findings after fixes; committed as `9886cb9` with `refactor: share battle audio and cue scheduling`.
...
- [x] **Step 1: Run GitNexus impact for `saveProfile`, profile read/recovery functions, `getRoomDeviceId`, `sendDeviceStatus`, `deliveryAckKey`, `enqueueDeliveryAck`, `retryPendingDeliveryAcks`, and their callers.** Warn because `saveProfile` is CRITICAL.
- [x] **Step 2: Write failing identity tests.** Cover Player ID and nickname switching both ways, nickname escaping/length, march range 5–120, per-room/per-surface keys, legacy Rally key compatibility, Defense prefill without silent registration, and 16px input class wiring.
- [x] **Step 3: Write failing ACK queue tests.** Cover device/profile/order/revision keying, retry backoff, generation changes, deadline cutoff, confirmation, cancellation, reconnect, and duplicate enqueue.
- [x] **Step 4: Implement both pure UMD modules.** Do not include DOM rendering, Rally role logic, or Defense order calculations.
- [x] **Step 5: Replace Rally profile and ACK helpers with adapters.** Preserve current server messages and localStorage compatibility. Run focused tests after each replacement.
- [x] **Step 6: Run `node --test test/battle-identity.test.cjs test/battle-delivery.test.cjs test/player-protocol-client.test.cjs test/classic-delivery-client.test.cjs test/mobile-input-font.test.cjs`; expect all pass.**
- [x] **Step 7: Run `npm test && npm run test:delivery && npm run test:triple`; then run GitNexus change detection and commit.** Verified and committed as `5ae0c84` (`refactor: share battle identity and delivery client`).
...
- [x] **Step 4: Run the new tests and confirm module/style failures.** Run `node --test test/battle-ui-contract.test.cjs test/battle-status.test.cjs test/battle-drawer.test.cjs`; expect missing module/token failures.
- [x] **Step 5: Implement semantic battle tokens and shared component styles.** Layer `battle-ui.css` as tokens, base accessibility, shared cards/status, drawer, then responsive overrides. Preserve existing font and brand variables through semantic aliases; do not restyle non-battle pages yet.
- [x] **Step 6: Implement the pure status projector.** Render text plus icon/shape for every state. The same readiness input object remains the source for visible status, `deviceStatus`, and ACK; the projector adds copy and manager metadata only.
- [x] **Step 7: Implement the pure drawer state machine and pointer controller.** Use Pointer Events and pointer capture; update one transform custom property during drag; animate only transform/opacity; allow a new pointerdown to interrupt the current settle; never write storage, send messages, or recalculate page data during motion.
- [x] **Step 8: Adapt the current Rally console as a compatibility proof.** Add a handle, visible close/back controls, and the three detents around the existing console without changing selection, Fire, Cancel, audio, or password validation. A successful password and `managerAuthenticated` remain in page memory after `close()`; reopening goes directly to Command without a password or socket/storage write. Only an explicit server-confirmed invalidation/logout path may call the existing lock behavior. Closing must not make an unselected manager eligible for ordinary JOIN cues; selected managers retain only their own personal cue while closed.
- [x] **Step 9: Run the browser interaction matrix.** Chromium, Firefox, and WebKit passed the drawer/core matrix including 320/375/390/430 portrait widths, 667/844/568 short landscape, saved-password pre-sound gating, inert/focus restoration, inline feedback, and no horizontal overflow.
- [x] **Step 10: Run `node --test test/battle-ui-contract.test.cjs test/battle-status.test.cjs test/battle-drawer.test.cjs test/mobile-input-font.test.cjs test/cancel-rally-selection.test.cjs test/commander-launch-monitor.test.cjs`, then `npm test && npm run test:triple`.** Final clean-export evidence: unit 560/560, Delivery 125/125, Triple 223/223, and Chromium/Firefox/WebKit core plus compatibility flows passed.
- [x] **Step 11: Run GitNexus change detection and commit.** Shared shell/tokens landed in `a5ed47f`; the independently approved Rally compatibility drawer landed in `7b53623` (`feat: add mobile rally commander drawer`).
...
- [x] **Step 1: Run GitNexus impact for `parseMarchSeconds`, `applyOwnProfileUpdate`, `applyPlayerMarchUpdate`, and the room-player export surface before adding any shared Defense guard.** Shared routing validation is CRITICAL (23 direct dependents, 68 total, 17 flows); no existing validator implementation was changed, and its Rally/player regressions remained mandatory.
- [x] **Step 2: Write failing timing/config tests.** The suite now proves the exact 3:00 default and 5–300/5–120 endpoints, rejects malformed values, and locks launch/impact/personal GO to zero artificial offset.
- [x] **Step 3: Write failing lifecycle tests.** Immutable roster/audience, exact derived-field normalization, terminal dominance, single-order authority, 64-entry replay protection, two-manager first-writer behavior, queued removal, completion, and cross-round replay are covered. Coordinated stored-field corruption fails closed.
- [x] **Step 4: Write failing delivery tests.** All five canonical outcomes, exact order/profile/device target validation, failure-to-scheduled recovery before GO, no scheduled downgrade, frozen Too-late, bounded ACK history, and per-profile aggregation are covered.
- [x] **Step 5: Implement pure functions only.** The Defense domain remains independent of Durable Object storage, WebSockets, UI, audio, and Rally policy.
- [x] **Step 6: Add Defense removal projection helpers to `room-player.js`.** Purge intent survives retry, touches profile/owner/devices together, scans at most 150 raw entries, uses exact validated keys and null-prototype maps, and cannot delete a same-PID replacement.
- [x] **Step 7: Run `node --test test/defense-domain.test.cjs test/defense-delivery.test.cjs test/player-domain.test.cjs`; expect all pass.** Final focused evidence is 39/39; the final broad gate at commit time was `npm test` 517/517, Delivery 125/125, and Triple 219/219.
- [x] **Step 8: Run GitNexus change detection and commit.** Two independent review rounds ended at Critical 0 / Important 0 / Minor 0; committed as `fc70924` with `feat: add defense timing and delivery domains`.
...
- [x] **Step 3: Write failing rollback-fence tests before enabling any Defense protocol.** Missing surface and explicit `rally` preserve current Rally behavior; explicit `defense`, empty surface, and unknown surface must never receive Rally state or reach Rally mutations. Old hibernated attachments with no surface migrate to Rally; attachments with an invalid stored surface fail closed. Cover a Defense first message delivered after hibernation without a preceding `fetch()` path.
- [x] **Step 4: Implement and promote the server-only rollback fence.** Committed as `9ea212a` (`refactor: fence battle surfaces before defense launch`), then deployed from a clean export to QA version `2932b809-4ad7-4238-aafc-fc082cedbd67`. Rally/fail-closed Defense smoke passed. Production was promoted as server-only version `80640bd5-4064-446a-895b-a4189cee6419` with zero asset uploads and byte-identical live HTML/JS/CSS; this version is `SURFACE_FENCE_VERSION_ID`.
- [ ] **Step 5: Write the full failing surface-isolation tests.** Connect Rally and Defense sockets to the same Room instance; assert Rally snapshots never include Defense keys, Defense ordinary snapshots never include Rally keys or full roster, and surface broadcasts reach only matching sockets. Open 100 Defense sockets and prove Rally presence does not change; open Rally sockets and prove Defense profile/device presence does not change.
- [ ] **Step 6: Add surface-filtered live socket/device helpers, then load and persist the five namespaces independently.** Existing `persist()` and `persistAll()` remain Rally-only; every existing Triple and reliable-delivery loop must explicitly select Rally sockets. Add focused Defense persistence methods so a Defense failure cannot roll back Rally state or vice versa.
- [ ] **Step 7: Split state projection and broadcasting.** Keep `snapshot()/stateMsg()/broadcast()` as Rally adapters during migration, add `rallySnapshot`, `defensePlayerSnapshot`, `defenseManagerSnapshot`, `stateMsgForSocket`, `broadcastSurface`, and bounded `sendDefenseDelta`. Close/error/presence paths broadcast only within the attached surface.
- [ ] **Step 8: Migrate the QA predicate to fixed room `qa`.** Change both `src/delivery.js` and the current Rally delivery-shadow adapter to accept exactly `qa` and reject every generated `qa-kvk-*` name. Verify `src/rally-mode.js` enables the QA Triple gate for `qa` through that shared predicate; update delivery/Triple/core-browser tests and add explicit cancel/remove cleanup between fixed-room scenarios before continuing.
- [ ] **Step 9: Restrict protocol families with explicit allowlists.** Rally allows only `setRallyMode`, `deviceStatus`, `deliveryAck`, `registerPlayer`, `updateOwnProfile`, `updateOwnMarch`, `setPlayerMarch`, the legacy `setMarch` migration path, `removePlayer`, `setConfig`, `cmd`, `stage`, `ready`, `sim`, `hb`, `hello`, and the existing reliable-delivery ingress messages `deliveryShadowHello`, `deliveryShadowProbeAck`, and `deliveryShadowAck`; its server may still emit `deliveryShadowProbe`, `deliveryShadowCommand`, and `deliveryShadowCancel`. Defense allows only its explicit profile, readiness, manager, config, order, ACK, `hb`, and `hello` messages. Any `deliveryShadow*` frame on Defense, or any other cross-surface operational message, receives `wrong_surface` and cannot mutate storage. Preserve the current Rally response for unknown/future `deliveryShadow*` versions and add focused regression assertions. Defense clients must not send any shared profile message until a valid `defenseState` handshake proves the server is surface-aware.
- [ ] **Step 10: Run `node --test test/room-surface.test.cjs test/surface-isolation.test.cjs test/room-delivery.test.cjs test/reliable-room-delivery.test.cjs test/player-protocol.test.cjs test/delivery-model.test.cjs test/delivery-shadow-client.test.cjs test/rally-mode.test.cjs test/triple-room.test.cjs`, then `npm test && npm run test:delivery && npm run test:triple && npm run test:kvk-core:all`; run GitNexus change detection and commit.** Commit with `refactor: isolate room traffic by battle surface`.
...
**Rally Manage replacement:**

```text
Manage → Players: searchable full roster, edit march, confirmed remove, staged role truth
Manage → Room: connected website devices summary, editable Kingdom 1/2 display names,
               independent Double/Triple controls, semantic 44px Copy room link button
ordinary page: no room-wide online counter and no full roster
command pane: selected captains only; both renamed kingdom labels remain visible
```

Kingdom names are presentation metadata scoped to Rally, normalized to 1–24 visible Unicode characters, revisioned, and broadcast to every Rally client. They never cross into Defense state. The Room tab uses website-verifiable wording (`Connected website devices`), not game-online or participation wording. Copying the room link is a real `<button>` with keyboard activation and a 44px hit region; no SVG text acts as a control.
...
- [ ] **Step 6: Replace the freed Rally Manage area with exactly `Players` and `Room` tabs.** Reuse the shared drawer and existing roster/editor/remove flows. Add revisioned Rally-only kingdom display names, per-kingdom Double/Triple controls, connected website-device summary, and a semantic 44px room-link copy button. All mutations require the existing retained page-session commander authentication, settle only after exact receipts plus canonical state, replay idempotently after reconnect, and broadcast immediately to all Rally clients. Preserve default labels when names are blank or legacy state has none.
- [ ] **Step 7: Add Rally Room/Players UI and protocol tests.** Cover two managers racing on kingdom names, stale revision rejection with preserved drafts, reconnect replay, both kingdom labels updating on ordinary and commander views, Double/Triple controls staying independent, room link keyboard/clipboard fallback, 320/375/390/430 and 200% text, focus restore between tabs/drawer detents, no online count on ordinary pages, and no Rally room metadata in any Defense snapshot/frame.
- [ ] **Step 8: Recreate the listed active tests under Rally/coordination names, update every listed helper import and package script, and delete their old files.** `test/rally-update.test.cjs` and `test/rally-update-wiring.test.cjs` already come from Task 13, so delete the two old update tests without creating duplicates. Make `test/support/qa-coordination.cjs` accept only fixed room `qa`; update all local suites to isolate state through a temporary Wrangler persistence directory and all remote suites to serialize/clean their own stable `qa-test-*` profiles. Fold both old QA Playwright configurations into the new `playwright.qa-rally-defense.config.cjs`. Keep `legacy-kvk-script-guard.test.cjs`, `test/support/legacy-kvk-script-guard.cjs`, and `test/fixtures/kvk-legacy-target.cjs` as intentional legacy-route coverage. The game term KvK may remain in gameplay guide content; generated URLs and product assets may not use `/kvk`.
- [ ] **Step 9: Add a source guard.** Fail if public Rally HTML/JS/CSS contains the deleted Defense IDs/functions, if homepage/generated links point to `/kvk`, if an SVG text node is interactive, or if an executable test helper creates/opens a generated `qa-kvk-*` room. Whitelist only Worker redirects, migration build fields, legacy localStorage reads, legacy-route tests, and negative guard fixtures that prove non-`qa` rooms are refused.
- [ ] **Step 10: Run the focused absence/naming/Room-tab tests and `npm test && npm run test:delivery && npm run test:triple && npm run test:rally-core:all`; expect all pass.**
- [ ] **Step 11: Run GitNexus change detection and commit.** Commit with `refactor: remove static defense and finish rally management`.
...
- [ ] **Step 4: Confirm the rollback target is the Task 8 surface-fence production version.** Record the current production version immediately before promotion and require it to equal `SURFACE_FENCE_VERSION_ID`; save it as `PREVIOUS_VERSION_ID`. A rollback target that does not understand explicit Defense surface is a release blocker because cached/open Defense clients could receive Rally state.
...
- [ ] Rally Manage has exactly `Players` and `Room`: kingdom names and each kingdom's Double/Triple mode update all Rally clients, connected counts are explicitly website-device counts, and the 44px room-link button works by touch and keyboard; none of this metadata enters Defense.
...
- [ ] Manager authentication is independent from drawer state: close/reopen performs no socket/storage write, unselected managers remain silent, selected managers retain only their personal cues, and only server-confirmed invalidation clears authentication.
...
- [ ] The production rollback target is the verified server-only surface fence; explicit Defense fails closed there and can never receive or mutate Rally state.
