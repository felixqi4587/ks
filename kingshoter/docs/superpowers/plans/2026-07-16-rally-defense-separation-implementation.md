...
- [ ] **Step 8: Adapt the current Rally console as a compatibility proof.** Add a handle, visible close/back controls, and the three detents around the existing console without changing selection, Fire, Cancel, audio, or password validation. A successful password and `managerAuthenticated` remain in page memory after `close()`; reopening goes directly to Command without a password or socket/storage write. Only an explicit server-confirmed invalidation/logout path may call the existing lock behavior. Closing must not make an unselected manager eligible for ordinary JOIN cues; selected managers retain only their own personal cue while closed.
...
- [ ] **Step 4: Implement and promote the server-only rollback fence.** Parse surface before loading Rally delivery, projecting state, or writing an attachment. Persist an immutable valid surface atomically; explicit Defense returns `defense_not_available` and no Rally frame/mutation until the later handler is enabled. Run the full Rally/Delivery/Triple/browser gate, GitNexus change detection, and commit `refactor: fence battle surfaces before defense launch`. Deploy that exact commit to QA, run Rally plus fail-closed Defense smoke, then deploy it to production with no Defense UI/assets and record its production version as `SURFACE_FENCE_VERSION_ID`. This is the only post-hotfix pre-release production checkpoint.
