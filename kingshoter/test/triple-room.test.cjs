...
test('real broadcast projects by socket build and merge-safe attachments retain room identity', async () => {
  ...
  const current = socket(2026071401, '00000000-0000-4000-8000-000000000011');
  h.room.room.live.commands[1] = {
  ...
  assert.equal(attachment.clientBuild, 2026071401);
...
test('fetch binds the canonical room, client build, and Reliable defaults before its first state send', async () => {
  ...
  try {
    ...
    await h.room.fetch({
      ...
      url: 'https://qa-kvk.invalid/api/ws?room=operation-room&clientBuild=2026071401'
    });
  } finally {
  ...
  assert.equal(attachmentAtFirstSend.clientBuild, 2026071401);
