...
test('the updater bootstrap and isolated delivery controller load in one build generation', () => {
  const updateIndex = html.indexOf('<script src="/kvk-update.js?v=2026071507"></script>');
  const appIndex = html.indexOf('<script src="/app.js?v=2026071507"></script>');
  const shadowTag = '<script src="/kvk-delivery-shadow.js?v=2026071507"></script>';
  const shadowIndex = html.indexOf(shadowTag);
  const rallyTag = '<script src="/kvk-rally.js?v=2026071507"></script>';
  const rallyIndex = html.indexOf(rallyTag);
  const kvkIndex = html.indexOf('<script src="/kvk.js?v=2026071507"></script>');
...
test('KvK cache assertions move atomically to the supported build', () => {
  ...
  for (const source of cacheSources) {
    ...
    assert.equal(source.includes('kvk.js?v=2026071507') || source.includes('kvk\\.js\\?v=2026071507'), true);
  }
});
