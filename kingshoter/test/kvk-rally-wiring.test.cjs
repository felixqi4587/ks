...
test('only an exact same-generation rally module enables canonical Triple', () => {
  ...
  assert.equal(loadRallyAdapter(complete(2026071601), { BUILD: 2026071601 }).tripleClientAvailable, true);
  assert.equal(loadRallyAdapter(complete(2026071302), { BUILD: 2026071601 }).tripleClientAvailable, false);
  assert.equal(loadRallyAdapter({ ...complete(2026071601), BUILD: undefined }, { BUILD: 2026071601 }).tripleClientAvailable, false);
  const hostile = complete(2026071601);
  Object.defineProperty(hostile, 'BUILD', { get() { throw new Error('mixed rally cache'); } });
  assert.equal(loadRallyAdapter(hostile, { BUILD: 2026071601 }).tripleClientAvailable, false);
});
...
test('myTarget rejects hostile module return shapes and uses the safe canonical fallback', () => {
  ...
  const context = loadRallyAdapter(hostile, { BUILD: 2026071601 });
...
test('socket build advertisement is current only for a complete shared runtime', () => {
  function advertised(tripleClientAvailable, KvkUpdate, rallyClientBuild = 2026071601) {
    let options = null;
    const context = {
  ...
  assert.equal(advertised(true, { BUILD: 2026071601 }), 2026071601);
  assert.equal(advertised(false, { BUILD: 2026071601 }), 0);
  assert.equal(advertised(true, { BUILD: 2026071601 }, 2026071302), 0);
  assert.equal(advertised(true, null), 0);
  assert.equal(advertised(true, { BUILD: '2026071601' }), 0);
  const hostile = {};
