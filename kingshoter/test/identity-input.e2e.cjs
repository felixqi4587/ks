...
(async () => {
  ...
  try {
    ...
    assert.equal(await racePage.locator('link[href="app.css?v=2026071504"]').count(), 1, 'the identity CSS cache version is exact');
    assert.equal(await racePage.locator('script[src="/app.js?v=2026071504"]').count(), 1, 'the shared socket cache version is exact');
    assert.equal(await racePage.locator('script[src="/kvk.js?v=2026071504"]').count(), 1, 'the identity script cache version is exact');
