...
(async () => {
  ...
  try {
    ...
    assert.equal(await racePage.locator('link[href="app.css?v=27"]').count(), 1, 'the identity CSS cache version is exact');
    assert.equal(await racePage.locator('script[src="app.js?v=10"]').count(), 1, 'the shared socket cache version is exact');
    assert.equal(await racePage.locator('script[src="kvk.js?v=37"]').count(), 1, 'the identity script cache version is exact');
