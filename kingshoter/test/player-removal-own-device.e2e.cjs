...
const room = `remove-own-${Date.now()}`;
const url = `${base}/kvk?room=${room}&notour=1`;
const meKey = `kingshoter_r_${room}_me`;

(async () => {
  const browser = await chromium.launch({ headless: true, channel: 'chrome' });
  const player = await (await browser.newContext({ viewport: { width: 390, height: 1000 }, locale: 'en-US' })).newPage();
  const manager = await (await browser.newContext({ viewport: { width: 390, height: 1000 }, locale: 'en-US' })).newPage();
  const errors = [];
  ...
  try {
    ...
    const playerReloaded = player.waitForEvent('load', { timeout: 5000 });
    await manager.locator('#roster .rpi[data-pid="001"] .rpdel').click();
    await playerReloaded;
    ...
    assert.equal(await player.locator('#roster .rp[data-pid="001"]').count(), 0, 'the removed profile stays absent after its own device reloads');
